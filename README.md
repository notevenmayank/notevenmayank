<div align="center">

  <!-- ================= RETRO PIXEL DEV BANNER (LOCAL REPO ASSET) ================= -->
  <img src="https://raw.githubusercontent.com/notevenmayank/notevenmayank/main/assets/header.gif" width="100%" alt="Retro Dev Setup" style="border-radius: 12px; max-height: 380px; object-fit: cover;" />

  <br/><br/>

  <!-- ================= HEADER TITLE CARD (LOCAL REPO ASSET) ================= -->
  <img src="https://raw.githubusercontent.com/notevenmayank/notevenmayank/main/assets/title.svg" width="100%" alt="Mayank Joshi - Full Stack Developer" />

  <br/><br/>

  <!-- ================= VISITOR BADGE & STATUS ================= -->
  <p align="center">
    <a href="https://github.com/notevenmayank">
      <
    </a>
    <img src="https://img.shields.io/badge/Status-Building%20Cool%20Things-brightgreen?style=flat-square&logo=github" alt="Status" />
    <img src="https://img.shields.io/badge/Focus-Full%20Stack%20%26%20AI-blueviolet?style=flat-square" alt="Focus" />
  </p>

  <!-- ================= SOCIAL BADGES ================= -->
  <p align="center">
    <a href="mailto:mankujoshi8319@gmail.com" target="_blank">
      <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" />
    </a>
    <a href="https://github.com/notevenmayank" target="_blank">
      <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
    </a>

    
   
  </p>

</div>

---

<!-- ================= QUICK NAVIGATION ================= -->
<details>
  <summary>📌 <b>Quick Navigation (Click to Expand)</b></summary>
  <ul>
    <li><a href="#-about-me">🙋 About Me</a></li>
    <li><a href="#-featured-project-spotlight">🌟 Featured Project Spotlight</a></li>
    <li><a href="#-tech-stack--tools">🛠️ Tech Stack & Tools</a></li>
    <li><a href="#-github-streak--metrics">📊 GitHub Streak & Metrics</a></li>
    <li><a href="#-connect--collaborate">📬 Connect & Collaborate</a></li>
  </ul>
</details>

---

## 🙋 About Me

```yaml
name: Mayank Joshi
degree: B.Tech in Computer Science & Engineering (2025–2029) from Graphic Era Bhimtal
interests: Full-Stack Engineering, Backend Architecture, Real-Time Systems, AI Function Calling,Problem Solving
current_focus: Building production-ready web apps & sharpening Data Structures & Algorithms
hobbies: Exploring open-source tech, retro gaming aesthetics, coffee & lo-fi coding sessions
```

- 🎓 **Education**: Pursuing **B.Tech Computer Science & Engineering from Graphic Era**
- 🚀 **Latest Engineering Feat**: Architected and deployed **[Sahayak Setu](https://github.com/notevenmayank/Sahayak-Setu)** — a cooperative fair-trade worker platform with real-time GPS tracking, 230 automated test suites, and Groq Cloud AI assistant.
- 💡 **Passions**: Clean code architecture, RESTful API design, database integrity, and operational safety systems.
- 🧠 **Problem Solving**: Sharpening algorithmic skills & problem solving on **LeetCode**.
- 🤝 **Collaboration**: Always excited to collaborate on impactful full-stack, AI, or open-source projects!

---

## 🌟 Featured Project Spotlight

<div align="center">

### 🛡️ **[Sahayak Setu (सहायक सेतु)](https://github.com/notevenmayank/Sahayak-Setu)**
*Intelligent Fair-Trade Worker Service & Safety Ecosystem*

[![Live Website](https://img.shields.io/badge/Live_Website-Render-46E3B7?style=for-the-badge&logo=render&logoColor=white)](https://sahayak-setu.onrender.com)
[![GitHub Repository](https://img.shields.io/badge/Repository-Sahayak_Setu-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/notevenmayank/Sahayak-Setu)
[![Tests Passing](https://img.shields.io/badge/tests-230_passed-success.svg?style=for-the-badge)](https://github.com/notevenmayank/Sahayak-Setu#-testing)
[![AI Engine](https://img.shields.io/badge/AI_Engine-Groq_Cloud-F05032?style=for-the-badge&logo=groq&logoColor=white)](https://groq.com/)

<br/>

| Key Architecture Highlights | Technology Stack |
| :--- | :--- |
| • **🤖 Groq Cloud AI Grounding**: 12 strictly verified SQLite database tools running `qwen/qwen3.8-27b`<br>• **📍 Live Worker GPS Tracking**: Authorization-protected endpoints (`403 Forbidden` guard) with MapLibre GL modal<br>• **🗺️ Complaint & Demand Heatmap**: Interactive GeoJSON city hotspot visualization<br>• **🛡️ Worker Protection Center**: Suraksha micro-insurance prototype + 1-touch Emergency SOS + fatigue scoring | • **Frontend**: Vanilla JS (ES6+), HTML5, CSS3, Tailwind, MapLibre GL<br>• **Backend**: Node.js, Express.js, JWT RBAC<br>• **Database**: SQLite3 (WAL mode)<br>• **Cloud**: Render, Groq Cloud API |

👉 **Try the Live App**: **[sahayak-setu.onrender.com](https://sahayak-setu.onrender.com)**

</div>

---

## 🛠️ Tech Stack & Tools

<div align="center">

### 💻 Languages & Scripting
<a href="https://skillicons.dev">
  <img src="https://skillicons.dev/icons?i=c,cpp,js,ts,py,html,css" alt="Languages" />
</a>

<br/>

### ⚙️ Backend, Frameworks & Databases
<a href="https://skillicons.dev">
  <img src="https://skillicons.dev/icons?i=nodejs,express,sqlite,postgres,tailwind,bootstrap" alt="Backend & DB" />
</a>

<br/>

### 🧰 Cloud, AI & Developer Tools
<a href="https://skillicons.dev">
  <img src="https://skillicons.dev/icons?i=git,github,vscode,postman,docker,linux,npm" alt="Tools" />
</a>

</div>

---

## 📊 GitHub Streak & Metrics

<div align="center">

  <!-- Streak Stats (Verified 200 OK) -->
  <img src="https://streak-stats.demolab.com?user=notevenmayank&theme=tokyonight&hide_border=true" alt="GitHub Streak" />

</div>

---

## 💭 Daily Dev Thought

<div align="center">
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" alt="Dev Quote" />
</div>

---

## 📬 Connect & Collaborate

<div align="center">

Got a project idea, open-source opportunity, or just want to chat about code & tech? Feel free to reach out!

[![Gmail](https://img.shields.io/badge/Email%20Me-mankujoshi8319%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mankujoshi8319@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)]([https://linkedin.com/](https://www.linkedin.com/in/mayankjoshi902700/))
[![GitHub](https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/notevenmayank)

<br/>

<sub><i>Designed with passion & precision by <b>Mayank Joshi</b>. Always learning, building, and evolving.</i></sub>

</div>
